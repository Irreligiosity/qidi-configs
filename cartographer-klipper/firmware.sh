	...
	if [[ $flash == "usb" ]] || [[ $flash == "" ]]; then
		...
		if [[ -d /dev/serial/by-id/ ]]; then
			...
			if [[ $usbCartoCheck == "Cartographer" ]]; then
				...
				cd ~/klipper/scripts
				~/klippy-env/bin/python3 -c "import flash_usb as u; u.enter_bootloader('/dev/serial/by-id/${cartoID}')"
				sleep 5
	...
	if [[ $flash == "can" ]] || [[ $flash == "" ]]; then
		...
		if [ -d ~/katapult ]; then
			...
			if [[ $canCheck != "" ]]; then
				...
				canbootCheck=$(~/klippy-env/bin/python3 ~/klipper/scripts/canbus_query.py can0 | grep -v 'Klipper$' | grep -v 'Total.*uuids found' | grep 'CanBoot')
				if [[ $canbootCheck != "" ]]; then
					# Save CanBoot Device UUID
					canbootID=$(~/klippy-env/bin/python3 ~/klipper/scripts/canbus_query.py can0 | grep -v 'Klipper$' | grep -v 'Total.*uuids found' | grep 'CanBoot' | awk -F'canbus_uuid=' '{print $2}' | awk -F', ' '{print $1}'
	)
					found=1
				fi	
				# Check for Canbus Katapult device
				katapultCheck=$(~/klippy-env/bin/python3 ~/klipper/scripts/canbus_query.py can0 | grep -v 'Klipper$' | grep -v 'Total.*uuids found' | grep 'Katapult')
				if [[ $katapultCheck != "" ]]; then
					# Save Katapult Device UUID
					katapultID=$(~/klippy-env/bin/python3 ~/klipper/scripts/canbus_query.py can0 | grep -v 'Klipper$' | grep -v 'Total.*uuids found' | grep 'Katapult' | awk -F'canbus_uuid=' '{print $2}' | awk -F', ' '{print $1}'
	)
					found=1
				fi
			fi
	...
	if [[ $canCheck != "" ]]; then 
		...
		if ! [[ $uuid == "b" ]]; then
			...
			if [[ $check2uuid == "Flash Success" ]]; then
				# Check for canboot device
				canboot2Check=$(~/klippy-env/bin/python3 ~/klipper/scripts/canbus_query.py can0 | grep -m 1 "CanBoot")
				sleep 5
				if [[ $canboot2Check != "" ]]; then
					# Save CanBoot Device UUID
					canboot2ID=$(~/klippy-env/bin/python3 ~/klipper/scripts/canbus_query.py can0 | grep -m 1 -oP "canbus_uuid=\K.*" | sed -e 's/, Application: CanBoot//g')
					found=1
					findqueryUUID=$uuid
					queryID=$canboot2ID
					if [[ $queryID == *"Klipper"* ]]; then
				...
				# Check for Canbus Katapult device
				katapult2Check=$(~/klippy-env/bin/python3 ~/klipper/scripts/canbus_query.py can0 | grep -m 1 "Katapult")
				sleep 5
				if [[ $katapult2Check != "" ]]; then
					# Save Katapult Device UUID
					katapult2ID=$(~/klippy-env/bin/python3 ~/klipper/scripts/canbus_query.py can0 | grep -m 1 -oP "canbus_uuid=\K.*" | sed -e 's/, Application: Katapult//g')
					found=1
					findqueryUUID=$uuid
					queryID=$katapult2ID
					if [[ $queryID == *"Klipper"* ]]; then
...
flashFirmware(){
	...
	if [[ -n $usbID ]] && [[ $2 == 3 ]]; then
		...
		cd "/home/mks/cartographer-klipper/firmware/v2-v3/katapult-deployer" || exit
		...
		if [[ $ftype == "katapult" ]]; then
			...
			# Set directory based on parameter
			if [[ $1 == 1 ]]; then
				cd "/home/mks/cartographer-klipper/firmware/v2-v3/survey" || exit
			else
				cd "/home/mks/cartographer-klipper/firmware/v2-v3/" || exit
			fi
			...
			archive_dir="/home/mks/cartographer-klipper/firmware/v2-v3/survey" # Corrected path
			if [[ -d $archive_dir ]]; then
				# Get the folder names sorted from largest to smallest version
				folders=($(ls -d "$archive_dir"/*/ | sort -rV))
...
flashing(){
	...
	if [[ $firmwareFile != "" ]]; then
		...
		if [[ $usbID != "" ]] && [[ $3 == "usb" ]] && [[ $flashed == 0 ]]; then
			...
			# FLash USB Firmware
			~/klippy-env/bin/python3 ~/klipper/lib/canboot/flash_can.py -f $firmwareFile -d /dev/serial/by-id/$usbID
			read -p "Press enter to continue"
...
For USB based probes\n\n"
	...
	printf "${RED}~/klippy-env/bin/python3 ~/klipper/scripts/canbus_query.py can0${NC}-\n"
